...
BEGIN
    ...
    INSERT INTO @s (transaction_number, transaction_date_sk, account_sk, customer_sk, amount, description, row_hash)
    ...
    SET    [$(DWH)].[fact].[Transaction].[transaction_date_sk] = [tv_s].[transaction_date_sk],
    ...
    WHERE  ([$(DWH)].[fact].[Transaction].[row_hash] <> [tv_s].[row_hash])
           OR ([$(DWH)].[fact].[Transaction].[row_hash] IS NULL
               AND [tv_s].[row_hash] IS NOT NULL)
           OR ([$(DWH)].[fact].[Transaction].[row_hash] IS NOT NULL
               AND [tv_s].[row_hash] IS NULL);
    INSERT INTO [$(DWH)].[fact].[Transaction] ([transaction_number], [transaction_date_sk], [account_sk], [customer_sk], [amount], [description], [row_hash])
    SELECT [tv_s].[transaction_number],
