    ...
    SELECT
        ...
        (
            ...
            CONVERT(VARBINARY(MAX),
                CONCAT_WS(N'|',
                    COALESCE(CONVERT(NVARCHAR(20),  [r].[n].value('(account_number/text())[1]', 'INT')), N''),
                    COALESCE(CONVERT(NVARCHAR(30),  TRY_CONVERT(DATE, [r].[n].value('(transaction_date/text())[1]', 'NVARCHAR(30)')), 126), N''),
                    COALESCE(CONVERT(NVARCHAR(60),  TRY_CONVERT(DECIMAL(19,4), [r].[n].value('(amount/text())[1]', 'NVARCHAR(60)'))), N''),
                    COALESCE(CONVERT(NVARCHAR(400), NULLIF([r].[n].value('(description/text())[1]', 'NVARCHAR(400)'), N'')), N'')
                )
            )
        ),
